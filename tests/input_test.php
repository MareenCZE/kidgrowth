...
{
    assert_close(122.5, rust_input_number('122,5'), 1e-9, 'comma decimal');
    assert_close(122.5, rust_input_number('122.5'), 1e-9, 'dot decimal');
    assert_close(1225.5, rust_input_number('1 225,5'), 1e-9, 'space thousands separator');
}

function test_input_number_rejects_garbage()
{
    assert_null(rust_input_number('12abc'), 'trailing letters');
    assert_null(rust_input_number('abc'), 'no digits at all');
    assert_null(rust_input_number(''), 'empty string');
    assert_null(rust_input_number('   '), 'whitespace only');
}
...
function test_input_number_zero_and_negative_are_not_a_measurement()
{
    /* A height or weight of exactly zero, or negative, is not a real
       measurement - rust_input_number() folds both into "not measured"
       (null), the same as an empty field, rather than storing a value no
       chart could sensibly plot. */
    assert_null(rust_input_number('0'), 'zero');
    assert_null(rust_input_number('-5'), 'negative');
}
...
function test_input_number_scientific_notation()
{
    ...
    assert_close(100000.0, rust_input_number('1e5'), 1e-9);
}

function test_input_number_very_long_string()
{
    assert_null(rust_input_number(str_repeat('9', 1000) . 'x'), 'long garbage does not crash or pass');
}

function test_valid_date_accepts_real_calendar_dates()
{
    assert_true(rust_valid_date('2026-01-19'));
    assert_true(rust_valid_date('2000-02-29'), 'leap day in a leap year');
}

function test_valid_date_rejects_wrong_shape()
{
    assert_true(!rust_valid_date('19-1-2026'), 'wrong field order/width');
    assert_true(!rust_valid_date('2026/01/19'), 'wrong separator');
    assert_true(!rust_valid_date(''), 'empty string');
    assert_true(!rust_valid_date('not a date'), 'not a date at all');
}

function test_valid_date_rejects_shaped_but_impossible_dates()
{
    /* These match the old regex (\d{4}-\d{2}-\d{2}) but are not real dates -
       exactly the gap rust_valid_date() (checkdate()-backed) closes. */
    assert_true(!rust_valid_date('2026-13-45'), 'month 13, day 45');
    assert_true(!rust_valid_date('0000-00-00'), 'the all-zero placeholder');
    assert_true(!rust_valid_date('2025-02-29'), '2025 is not a leap year');
}
