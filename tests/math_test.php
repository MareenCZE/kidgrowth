...
function test_percentile_known_quantiles()
{
    /* The exact tail percentiles SZU publishes, and the point everything is
       anchored to. */
    assert_close(3.0, growth_percentile(-1.8807936081512509), 0.001, 'z=-1.8808 -> P3');
    assert_close(50.0, growth_percentile(0.0), 1e-9, 'z=0 -> P50');
    assert_close(97.0, growth_percentile(1.8807936081512509), 0.001, 'z=+1.8808 -> P97');
}

function test_percentile_null_propagates()
{
    assert_null(growth_percentile(null));
}
...
function test_lms_at_monotone_and_bounded()
{
    ...
    assert_null(growth_lms_at($rows, -0.5), 'below range');
    assert_null(growth_lms_at($rows, 2.5), 'above range');
    /* Exactly on a published row returns that row's own values, untouched. */
    $exact = growth_lms_at($rows, 1.0);
    assert_equals(75.0, $exact['m'], 'exact row returns its own M');
}

function test_decimal_age_basic()
{
    assert_close(1.0, growth_decimal_age('2019-01-19', '2020-01-19'), 0.01, 'non-leap year span');
    /* 2000 was a leap year, so this span is 366 days, not 365. */
    assert_close(366 / 365.25, growth_decimal_age('2000-01-01', '2001-01-01'), 1e-9, 'leap year span');
    assert_close(0.0, growth_decimal_age('2020-06-15', '2020-06-15'), 1e-9, 'same day');
}

function test_bmi()
{
    /* 20 kg at 100 cm -> BMI 20.0 exactly, a round number worth pinning. */
    assert_close(20.0, growth_bmi(100.0, 20.0), 1e-9);
    assert_null(growth_bmi(0, 20.0), 'zero height');
    assert_null(growth_bmi(100.0, 0), 'zero weight');
    assert_null(growth_bmi(null, 20.0), 'null height');
}
...
function test_target_height_both_sexes()
{
    ...
    assert_null(growth_target_height(null, 165.0, 'm'), 'missing father height');
}

/**
 * The "70 cm bug": growth_channel_projection() used to clamp
