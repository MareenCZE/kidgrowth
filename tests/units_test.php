 ...
 * file exists.
 *
 * Runs each case in a fresh CLI process (growth_units() memoises per request)
...
function growth_test_run_units_snippet($units, $code)
{
...
function test_units_defaults_to_metric()
{
    $out = shell_exec(escapeshellarg(PHP_BINARY) . ' -r '
        . escapeshellarg('require ' . escapeshellarg(dirname(__DIR__) . '/src/data.inc') . '; echo growth_units();'));
...
function test_display_length_round_trip()
{
    ...
    $out = growth_test_run_units_snippet('imperial', 'echo growth_display_length(100.0);');
    assert_close(39.3700787, (float)$out, 1e-5);
}

function test_display_length_metric_is_unchanged()
{
    $out = growth_test_run_units_snippet('metric', 'echo growth_display_length(100.0);');
    assert_equals('100', $out);
}

function test_length_unit_label()
{
    assert_equals('cm', growth_test_run_units_snippet('metric', 'echo growth_length_unit();'));
    assert_equals('in', growth_test_run_units_snippet('imperial', 'echo growth_length_unit();'));
}

function test_display_weight_round_trip()
{
    /* 1 kg is about 2.2046 lb. */
    $out = growth_test_run_units_snippet('imperial', 'echo growth_display_weight(1.0);');
    assert_close(2.2046226, (float)$out, 1e-5);
}

function test_parse_length_metric_is_plain_cm()
{
    $out = growth_test_run_units_snippet('metric', "echo growth_parse_length_input('122,5');");
...
function test_parse_length_imperial_feet_and_inches()
{
    ...
    foreach (array("4'3\"", "4' 3\"", "4'3", "4ft3in", "4 ft 3 in") as $input) {
        $out = growth_test_run_units_snippet('imperial', "echo growth_parse_length_input(" . var_export($input, true) . ");");
...
{
    $out = growth_test_run_units_snippet('imperial', "echo growth_parse_length_input(\"4'\");");
...
function test_parse_length_imperial_decimal_inches_with_fraction()
{
    ...
    $out = growth_test_run_units_snippet('imperial', "echo growth_parse_length_input(\"4'10.5\\\"\");");
...
function test_parse_length_imperial_bare_number_is_inches()
{
    ...
    $out = growth_test_run_units_snippet('imperial', "echo growth_parse_length_input('48.2');");
    assert_close(48.2 * 2.54, (float)$out, 1e-6);
}

function test_parse_length_rejects_garbage()
{
    $out = growth_test_run_units_snippet('imperial', "var_export(growth_parse_length_input('not a height'));");
    assert_equals('NULL', $out);
}

function test_parse_weight_metric_is_plain_kg()
{
    $out = growth_test_run_units_snippet('metric', "echo growth_parse_weight_input('20,5');");
...
{
    /* 45 lb -> kg */
    $out = growth_test_run_units_snippet('imperial', "echo growth_parse_weight_input('45');");
...
function test_lms_conversion_is_scale_invariant()
{
    /* The whole justification for converting M alone (units.inc's
       growth_display_chart_rows() docblock): z-score computed against a
    ...
    $metricZ = growth_zscore(103.0, $lms);

    $factor = 1.0 / GROWTH_CM_PER_INCH;
    $lmsInches = array('l' => $lms['l'], 'm' => $lms['m'] * $factor, 's' => $lms['s']);
    $inchesZ = growth_zscore(103.0 * $factor, $lmsInches);
